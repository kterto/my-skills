...
12. **Release-readiness derivation (no new stored state):**
    ...
    - `READY(r) :=` **every not-superseded story with `release=r` is `done`, regardless of system** — i.e. no cell in row `r` has remaining not-done work across **every column: each declared-system column, the `(untagged)` column, and the `(unknown)` column when present**. Untagged and orphan (`(unknown)`) work therefore **blocks** readiness exactly like declared-system work — the rule is release-wide, not quantified over declared systems only. `superseded` counts as "no remaining work" exactly as in the existing rollup.
    - Untagged (`system: null`) stories appear in an `(untagged)` column, and non-null-but-undeclared (orphan) stories in an `(unknown)` column, so nothing is dropped. Rows are the named releases (`releases[]` order) + a single `(untiered)` row; `backlog` is **not** a row (parked work is not a shippable release). The matrix is recomputed on demand.
