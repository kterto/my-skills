...
| `max_run_minutes` | integer ≥ 0 (finite) | `0` (off) | — |
...
`max_run_minutes` is the run's only **cost** meter. Every other budget here counts cycles or lane width, which is a proxy for spend that stops tracking it the moment one cycle costs ten times another: the run this key was written for reached all three of its cycle caps and still ran fifteen hours, because nothing anywhere could answer "how long have I been going". Bound at Step 0b from `boundaries[0].at` in the verification ledger and evaluated at every boundary the run already mints, it prints elapsed minutes beside each one and, when the budget is spent, stops with a `STALLED` banner rather than opening another cycle. Default `0` disables it, because a number chosen badly here stops good runs — set it once you have a baseline for what your runs actually cost. It is a **stop**, not a kill: work already done stays on disk and the report says what remained.
...
| `max_run_minutes` | a **finite integer ≥ 0** | `0` disables the run meter, which is the default and the pre-existing behavior. A **negative** budget is spent before the run starts, so it would stop every run at its first boundary — the intent it expresses is already `0`'s, without `0`'s explicit disable. |
