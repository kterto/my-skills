...
**Enforced at the schema boundary, not by informal Phase-3 code.** The runtime validator
(`validate-subagent-return.cjs`) takes the catalog in its `ctx` as **ownership maps**
(`{ catalog: { entityOwner: {id: moduleId}, nodeOwner: {id: moduleId}, moduleIds: [owned…] } }`;
flat `entityIds`/`nodeIds` sets are still accepted for membership-only, back-compat). With
ownership it distinguishes:

- **Declaration** (`entities[].id`, `dataFlowEdges[].fromId`) — the ids a unit *defines*: must
  be **owned by the unit** (`moduleIds` contains the id's owning module), or a `new:<owned>:…`
  id. A subagent cannot declare another module's identity.
- **Reference** (`entities[].relations[]`, `dataFlowEdges[].toId`) — the ids a unit *points at*:
  may be **any known catalog id** (a foreign target is a legitimate cross-module link), or a
  `new:<owned>:…` id.

`crossModule` on a stitched edge is derived from ownership: `owner(fromId) ≠ owner(toId)`.
Every `new:<module-id>:<name>` is still bound to an owned module. All of this is checked in the
same executable contract as every other field, before merging.
