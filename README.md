...
On the first run (or when `.orchestrator/config.json` is absent, or when any file bootstrap materializes has gone missing) the skill runs **bootstrap** automatically: it scans the repo, interviews you about missing context until confidence ≥ `context_threshold`, writes `.orchestrator/PROJECT-CONTEXT.md`, renders the six role templates into the host agent directory (`.claude/agents/` in Claude Code, `.opencode/agent/` in opencode), and writes `.orchestrator/config.json`. **An existing `PROJECT-CONTEXT.md` is never overwritten** — a re-run on an upgrade reports which required headings it is missing and leaves the curated file alone. That context file carries a size budget (target 12 KB, 20 KB as the move-it-out line) because every role re-reads it at every spawn: keep the rule inline and link the reasoning out. Subsequent invocations skip straight to the pipeline.

Bootstrap also **materializes the load-bearing reference files** into `.orchestrator/` — `artifact-format.md`, `config.md`, `lane-protocol.md`, `gate-config.md`, the seven `html-templates/*.template.html` scaffolds, and four zero-dependency Node scripts (`render-artifact.cjs`, `check-artifact-pairing.cjs`, `check-artifact-links.cjs`, `gate-scope.cjs`) — because subagents cannot read the skill's own directories. They are re-copied on every bootstrap so they track the installed skill version.

Three further files in `.orchestrator/` are **written at run time** by the orchestrator, not copied by bootstrap: `verification-ledger.json` (created at Step 0a on every run — the tree hash at each boundary, plus one row per whole-app suite execution), `run-manifest.json` (written at Step 2c on a parallel run — the provenance anchor `--resume` validates against), and `join-digest.md` (built at Step 3j.4 on a parallel run when `join_digest` is `true`).
...
brainstormer → architect → coder → tester → reviewer ──(APPROVED)──→ qa ──(READY_TO_COMMIT)──→ report
                              ...
                              └── REQUEST_CHANGES loop ─┘                   └── BLOCKED loop ──┘
                                  (review_budget)                               (max_qa_cycles)
```

**The review cap is a budget, not the raw key.** At Step 0b the orchestrator binds `review_budget = min(max_review_cycles, max_family_cycles − family_cr_count)`, and every cycle test in the run tests against that, never against the raw key. `--max-review` raises the in-run bound; nothing raises the family remainder, which is what `--override-family-budget` exists for. The stall banner names which of the two bound it, on a `Bound by:` line — without it the two stalls print identically and you pick a remedy by guessing.

**The run records its tree over time.** Step 0a mints a tree hash (`git write-tree` in an isolated index, `plans/` and `.orchestrator/` excluded) and creates `.orchestrator/verification-ledger.json`; the hash is re-minted at every boundary the tree stops moving — Steps 3, 3j, 4c and 5d. Step 0d then takes the optional **baseline sweep** (`baseline_sweep`): one run of every whole-app suite and gate command against that untouched tree. It blocks nothing — a red baseline is the expected state of a real repository — and it is what later lets a role label a finding *pre-existing (baseline)* instead of re-deriving the answer from `git show`, independently, in every artifact.

The ledger's other half is **suite inheritance**. The tester, QA and the outer join each append a row for every whole-app suite they execute, naming the command verbatim and the tree hash it ran against; before running one, they look for a row matching **both** exactly. A recorded `pass` is inherited rather than re-executed; a recorded `fail` never is — it is re-run once, and a second disagreeing result is recorded as `flake`. It narrows nothing: every suite that would have run still runs, once per tree.

**Every plan must account for the gates.** The architect closes each plan's `## Verification (per phase)` section with a `### Gate coverage` table — one row per runtime gate id, each naming the command that carries it or a reasoned `n/a` — plus a `| Command | Scope | Phases | Path condition |` table whose every row carries one token from the closed set `changed-files | whole-project | advisory-instrument | deferred-to-join`. The orchestrator verifies both before any coder starts. **A whole-app test suite is banned at phase exit in every mode**, sequential and parallel alike: it belongs to the run's barriers, so a `whole-project` row naming a test runner is a malformed plan.

**The reviewer discloses its read scope.** From review cycle 2 the orchestrator passes a `delta=` line — the files that cycle changed, diffed between two ledger boundaries — and the CR gains a `## Read scope` section stating what was actually examined. It narrows nothing: `MAESTRO_REVIEW_BASE`, the working-tree snapshot and the reviewer's read set are all unchanged. Reporting the scope is a strict increase in information over reporting none.

The skill never commits or pushes.

### Parallel lane execution (opt-in)

With `parallelism` ≠ `off`, the single architect→coder pair is replaced by a **leaf** fan-out that rejoins before the tester. The cycle caps, the eval and the final report are identical in both branches; the tester, reviewer and QA are the same roles making the same verdicts, but on the parallel path they read the run's aggregate from the **join digest** (`join_digest`, Step 3j.4) rather than walking the leaf plans, contracts and progress logs themselves.
...
| `2L` / `3L` | One architect then one coder **per leaf**, concurrent, isolated by disjoint path ownership in one shared workspace. `2L` checks requirement coverage over the leaf **union** and each leaf's `### Gate coverage` table **independently**, before any coder starts. `3L` is a single flat dispatch over the whole leaf set |
...
| `3j` | Outer join: wait for every leaf, verify every parent-contract row on both sides, run the top-level integration lane, `simplify` once over the union, re-run the leaf phase gates de-duplicated on the resolved command string, run every deferred gate once, mint the run's implementation boundary, build the **join digest** (`3j.4`), then hand the parent `PACT` ID to tester/reviewer/qa |
...
**Resume.** A halted parallel run is detected from `.orchestrator/run-manifest.json` (a provenance anchor, not a `plans/**` scan). Without `--resume` you get a single non-blocking hint and a fresh run; with `--resume`, completed leaves are preserved in place — no stash, reset, or clean — and only the incomplete leaves re-dispatch. Validation covers the branch, the pre-flight base, the spec's SHA-256, every `contract_ids` / `leaf_ids` / `integration_leaf_ids` entry, **and the manifest's `parallelism` against the level this invocation resolved** — a run whose config moved from `full` to `off` between sessions would otherwise re-enter a fan-out the resolved level forbids. Any mismatch or ambiguity aborts rather than guessing.
...
| `max_review_cycles` | `10` | `--max-review` | Upper bound on architect→coder→reviewer cycles. The cap actually tested is `review_budget = min(max_review_cycles, max_family_cycles − family_cr_count)`, bound at Step 0b |
| `max_eval_cycles` | `2` | — | Max in-loop spec-eval cycles (Step 4e) before the pipeline hard-stops |
| `max_family_cycles` | `6` | — | Max code reviews one **run family** — every artifact answering the same `SPEC-*`, across every run that touched it — may accumulate before the orchestrator refuses to start another. The only budget that survives a new run; `--override-family-budget` continues past it for one invocation, on the record |
...
| `baseline_sweep` | `"auto"` | — | `auto` \| `always` \| `off` — Step 0d's one sweep of every whole-app suite and gate command against the untouched tree, so a later role can tell a pre-existing red from one this run caused. `auto` runs it when the resolved `parallelism` is not `off`. Advisory in every mode: it records and prints, and never blocks |
| `join_digest` | `true` | — | Parallel path only: materialize the run's aggregate once per boundary into `.orchestrator/join-digest.md`, which tester/reviewer/QA read **in place of** the leaf plans, contracts and progress logs. Fails closed — a failed integrity check means no digest, no `aggregate=` line, and the roles read exactly what they read without it |
...
- **spec-driven-eval** skill — invoked at **Step 4e**, on every reviewer `APPROVED` and before QA, to grade the aggregate against the original spec, bounded by `max_eval_cycles`. The orchestrator passes `Profile: in-loop` — keeping the evidence matrix, the numeric `Final`, the ranked gaps and the Engineering Gates, and omitting `R`, `S`, `D` and the comparison roll-up, none of which this pipeline reads and none of which is folded into the grade — plus the verification ledger and the current tree hash, so a gate already recorded against that exact tree is scored from the record rather than re-executed. Fewer sections, never a laxer method. It ships in this marketplace (vendored, CC-BY-4.0 — see [its `UPSTREAM.md`](./plugins/my-skills/skills/spec-driven-eval/UPSTREAM.md)), so installing the plugin satisfies it on all three hosts. Bootstrap B2 resolves the bundled copy first, then any separately installed one, and only offers the upstream `npx` install when neither resolves. The pipeline degrades gracefully if the skill is absent (eval step is skipped with a warning).
- **simplify** skill — the mandatory pre-review cleanup pass at sequential Step 3 (scoped `--plan <FEAT-id>`) and at the parallel outer join Step 3j (scoped to the union diff, once per run). Both sites log a `SIMPLIFY` entry **carrying its `Bugs:` lines verbatim** — the correctness issues it observed and deliberately did not fix, each with a `file:line` — into the plan's `.progress.md` at Step 3 and the parent contract's `.progress.md` at Step 3j, which is the log the reviewer opens first. They travel as observations, never verdicts. It ships in this marketplace, so installing the plugin satisfies it on all three hosts; a host-provided `simplify` satisfies it equally. Bootstrap B2 records availability, and both steps degrade explicitly — printing `SIMPLIFY skipped — no simplify skill available` — rather than skipping silently or having the orchestrator do the pass itself.

The three scan steps (Bootstrap B1's context digest, Step 2p.1's slicing analysis, Step 3j.4's leaf-derived digest regions) spawn a **read-only scan subagent**, resolved per host in order: `Explore` → `explore` → `general-purpose` / `general`. If none exists, the scan runs inline in the orchestrator's own context rather than failing — a scan-subagent failure is never a parallelization verdict, and never a reason to skip the join digest.
