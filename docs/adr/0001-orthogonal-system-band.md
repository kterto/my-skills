...
- **`path` is untrusted input (injection boundary).** Because `roadmap.config.json` is contributor-editable and PM surfaces `path` into a command-capable agent's prompt, `path` must be **validated as a normalized repo-relative token** (no absolute/`..`/control-char/newline) on write and re-validated on read, and passed only as **clearly delimited untrusted metadata the agent must not interpret as instructions**. Any future use of `path` (routing, tooling) inherits this obligation. See `roadmap/references/config.md` → `path` validation and `product-manager/SKILL.md` → System context block.
