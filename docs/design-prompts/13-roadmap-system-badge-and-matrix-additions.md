...
- **HTML:** a `<section data-role="readiness-matrix">` containing a `.readiness` scroll wrapper and the `{{readiness_matrix}}` `<table>` (header `release` + declared systems + `(untagged)` + `READY?`; rows in `releases[]` order + a single `(untiered)` row — no `backlog` row, since parked work is not a shippable release; cells `done/total`).
...
- The section **always renders** — it is **never omitted**. For a legacy roadmap with no declared systems and nothing tagged it **collapses to a single `(untagged)` column** (rows = the `releases[]` + the `(untiered)` row — no `backlog` row; a release-less roadmap shows just the `(untiered)` row). The `(untagged)` column is never dropped (locked backward-compatibility contract — matches the invariant in the Parity/robustness checklist: "the embedded matrix always keeps a column for `system: null`"). The *separate* standalone `release-matrix.<ext>` dashboard FILE is gated (materialized only once ≥1 system/tag exists); this embedded index section is unconditional.
