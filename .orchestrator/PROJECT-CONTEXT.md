...
- **Lane B gates** (real, runnable, and hard floors when a change touches `plugins/my-skills/skills/**`, `prime-agent/overlays/**`, `prime-agent/skills/**`, or their scripts):
  ...
  - `cd prime-agent && npm test` — `tests/install.sh` + `tests/bootstrap.sh` + `tests/parity.sh` (installer containment/rollback, the `scripts/install-prime-agent.sh` bootstrap driven over a local clone source, builder guard rails, frontmatter/mode parity, and section 4's emitted-fence linter assertions against the pinned fixtures in `scripts/__tests__/fixtures/prime-fences/`).
