...
`max_eval_cycles` bounds how many times the in-loop spec eval (SKILL.md Step 4e) may fire in one run. The eval runs on every reviewer `APPROVED` and, on `ISSUES`, remediates through the ordinary review loop — so it shares and consumes the run's `review_budget` — `max(1, min(max_review_cycles, max_family_cycles − family_cr_count − eval_reserve))`, bound at Step 0b, which reserves this key's own value out of that budget so eval remediation cannot be raised against a family with nothing left to spend — and this key exists only to stop the eval itself from becoming the thing that never converges. Reaching it is a `STALLED` stop reported to the user, not a silent ship: repeated `ISSUES` on the same criteria means the spec and the implementation disagree in a way no further remediation run will settle. The default of `2` is deliberately tight — one eval, one chance to remediate. Raise it only with evidence that a third pass converges.

`max_run_minutes` **caps** the run's only **cost** meter — it does not switch the meter on. Elapsed
minutes are computed and printed at every boundary mint and on every terminal banner (`SKILL.md` →
*The run meter*, Step 7b `Run cost:`) whatever this key is set to; `0` means *do not stop*, never *do
not measure*. Reporting costs a subtraction against `boundaries[0].at`, which Step 0b binds
unconditionally, and a project cannot choose a bound here until it has seen what its runs actually
cost. The meter itself: Every other budget here counts cycles or lane width, which is a proxy for spend that stops tracking it the moment one cycle costs ten times another: the run this key was written for reached all three of its cycle caps and still ran fifteen hours, because nothing anywhere could answer "how long have I been going". Bound at Step 0b from `boundaries[0].at` in the verification ledger and evaluated at every boundary the run already mints, it prints elapsed minutes beside each one unconditionally and, when a budget is set and spent, stops with a `STALLED` banner rather than opening another cycle. Default `0` disables **the stop** — not the number — because a bound chosen badly stops good runs; the elapsed line is what gives you the baseline to choose one from. It is a **stop**, not a kill: work already done stays on disk and the report says what remained.
...
`join_digest` governs whether the orchestrator materializes the run's aggregate once per boundary for the three join roles to read in place of the leaf plans, contracts and progress logs (`references/parallel.md` → 3j.4). It is read on the **parallel path only** — a sequential run has no aggregate worth digesting, and emitting the line there would break the `off`-run prompt-parity invariant to buy nothing. Default `true`, because the mechanism **fails closed**: the orchestrator's integrity check runs before the tester is dispatched, and any failure means no digest is written, no `aggregate=` is emitted, and the roles read exactly what they read today — the downside of a bad build is a run that costs what it costs now, printed, not a gate that silently weakened. Set it `false` to compare two runs, or to take the digest out of the picture while diagnosing a verdict you distrust. **On the first run with it on, read the printed `DIGEST —` counts**: they are the whole audit surface, and a checkmark in their place would be worth nothing.
...
**Validation happens at config resolution (Step 0b), before any dispatch, and fails closed.** A value outside its range does **not** halt the run and is **never clamped silently** — it resolves to the key's **canonical default** — `max_parallel_lanes` `6`, `max_family_cycles` `6`, `max_eval_cycles` `2`, `max_contract_amendments` `2`, `max_run_minutes` `0`, `gate_wall_clock_minutes` `15`, `max_spec_requirements` `0` — with the reason printed:
...
| `--override-spec-size` | Skip Step 2's scope band for one invocation. Maps to no config key — a per-invocation intent, like `--override-family-budget`. Recorded in the plan's `.progress.md`. |
...
`lanes`, `sublanes`, `max_parallel_lanes`, `max_contract_amendments`, `max_eval_cycles`, `max_family_cycles`, `max_run_minutes`, `gate_wall_clock_minutes`, and `max_spec_requirements` have **no CLI arg** that sets their value (`--override-family-budget` and `--override-spec-size` bypass their gates for one run; neither changes the key) — set them in `.orchestrator/config.json` directly.
