...
Three steps admit a **scan** child rather than a pipeline role: Bootstrap B1 (context digest), Step 2p.1 (slicing analysis), and Step 3j.4 (the join digest's leaf-derived regions). It is admitted exactly like any other child — `handle = await rlm(prompt, name=…)` — and, unlike the six roles, it is **not materialized by B3**: it has no role file under `.orchestrator/roles/`, so its whole brief is the prompt you build for it. There is no host agent type to resolve; there never is under Prime Agent.
...
**Give each scan child a stable, per-caller `name`.** The three callers use **three different** names — `context-scan` for Bootstrap B1's context digest, `slicing-scan` for Step 2p.1's slicing analysis, `digest-scan` for Step 3j.4's leaf-derived regions — so a child that returns nothing usable can be addressed on retry with `receiver_name=handle.name`. One name shared across dispatch sites would leave a retry ambiguous about which scan it was re-asking, which is exactly the failure a stable per-child name exists to prevent.
