...
if [ -n "$(git -C "$root" status --porcelain -- . ':(exclude).opencode' ':(exclude).claude' ':(exclude).prime')" ]; then
...
2. **Run the orchestrator.** **Load the protocol once per session, not once per story.** **Story 1 of a PM run always invokes the `orchestrator` skill** — the check below applies from story 2 onward, and on story 1 the answer is known in advance.
   ...
   **If you can quote them, do not load the skill again**: start a new pipeline run directly from its Lifecycle, with this story's `## Brief` section as the task input. Otherwise load the `orchestrator` skill through whatever mechanism this host provides — a fresh session, or one where compaction has evicted it.
   ...
   Either way the story's `## Brief` section is passed to that run verbatim as the task input. Do not summarize, rewrite, or trim the brief — it is an orchestrator-ready contract and must be handed over exactly as written.
...
4. **Execute the success-path sequence** (see `references/git-flow.md` → **Success-path sequence**). The ordering is load-bearing because of two constraints: `/roadmap sync` must run after the trailer commit (it reads `git log`), and PM's own logs must be committed *after* the PR (they record the PR URL). The steps (the html-mode parity gate runs after sync, before the sync-docs commit):
   ...
   - Run the **human-validation check** (step 5), render `templates/pr-body.template.md` (its `{{human_validation_note}}` comes from that check; its `{{not_delivered}}` is the orchestrator banner's `Delivered:` / `Unmeasured:` / `Instrument moved:` / `Deferred by decision:` / `Issues found:` lines copied verbatim, never re-derived or summarized), open the PR with `gh pr create --body-file`, and capture the PR URL.
