...
| `rigor` | `sketch` \| `delivery` \| `hardened` | The level the orchestrator reported for this story, read from its terminal banner's `Rigor:` line. Recorded on every row, `hardened` included — a green at `sketch` and a green at `hardened` are different results, and this column is where that difference survives. Absent on rows written before the column existed; treat a missing cell as unknown, never as `hardened`. |
