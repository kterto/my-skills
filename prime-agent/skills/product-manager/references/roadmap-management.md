...
| `set-rigor <level> <selection>` | `rigor <level> <ids…>` | Set the **rigor** band on the resolved id set — what a green on those stories claims. `<level>` ∈ `sketch` \| `delivery` \| `hardened` \| `null` (clear). The vocabulary is **closed**, so an unknown value stops and prints the three levels; there is nothing to create lazily and nothing to declare. **A selection that lowers any story's level is confirmed before staging**, naming what stops applying at the new level, and writes one audit row per story with its direction — never a summary line. PM forwards the band, never chooses it (ADR-0024). |
