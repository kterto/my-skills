...
> **Step 2p is conditional — if it fires, PM answers it.** The orchestrator's Step 2p parallelization ladder (orchestrator `SKILL.md` → Step 2p) does **not** run when `parallelism` resolves to `off`, which is the default and therefore the normal PM run. But the key is also readable from the project's `.orchestrator/config.json`, and a project that set it to `ask` would surface the ladder inside a PM-driven run — PM does not set `automation_level`, so the autonomous no-prompt guard (Step 2p.4) does not cover it. The options are: 1. sequential (`off`), 2. lane-parallel implementation (`lanes`), 3. **nested lane-parallel (`full`)** — everything `lanes` does plus a sub-contract and concurrent sub-lane coders for each lane that clears the orchestrator's marginal-gain gate. **Option 3 may be omitted** from the ladder entirely when no lane clears that gate, in which case the question offers only options 1 and 2; PM's answer is unaffected either way. If the prompt appears, PM answers it on the user's behalf with **option 1, sequential (`off`)** — the same way it answers Step 0 — because PM's per-story contract is one story → one orchestrator run → one commit on `pm/<id>-<slug>`, and lane fan-out is a within-run concern the single up-front confirmation (Pre-flight step 8) never authorized. A project that explicitly configures `lanes` or `full` is **opting in** and does not break that contract: the orchestrator still ends at one `READY_TO_COMMIT` over one union diff, which is the only thing the success-path sequence below depends on. This is a documentation note about answering a prompt — PM's command surface is unchanged and PM passes no parallelism flag.

> **Step 0r resume detection — there is nothing for PM to answer.** The orchestrator detects a prior parallel run that halted `PARTIAL` and can resume only its incomplete leaves (orchestrator `references/parallel.md` → Step 0r). Unlike the Step 2p ladder above, **Step 0r never prompts** — not in `manual` mode, not on any host, not ever. It has exactly **two outcomes, no third**: `--resume` was passed, so resume is applied; or it was not, so the orchestrator prints a single **non-blocking hint** and starts a fresh run. **PM passes no `--resume` flag**, so a PM-driven run always takes the second outcome and always starts fresh. That is the outcome PM wants, for the same reason it answers Step 0 with "use this branch": PM's per-story contract is one story → one orchestrator run → one commit, and silently re-entering a prior run's frozen contract would produce a diff the current story's brief never authorized and a commit whose `Roadmap-Story:` trailer misdescribes it. **PM is never blocked by this, and the guarantee is structural rather than an answer PM has to prepare** — there is no question to be blocked by, so it does not depend on `automation_level`, on host capability, or on PM getting a reply right. PM's command surface is unchanged and it passes no resume flag.
