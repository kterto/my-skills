...
3. **Write** — on approval, apply the file writes: update item frontmatter/body, append audit rows, update `roadmap.lock.json` (including lazy `releases[]` creation). **Then re-render the readiness views** whenever this op changed a **readiness input** — a story's `status`, `release`, or `system`; the `releases[]` order/names; or the `config.systems` set — per the refresh rule in `SKILL.md` → Release readiness: the **embedded index matrix always** (it is unconditional), and the **standalone `/roadmap/release-matrix.<ext>` dashboard when it exists or the op first warrants it** (the standalone file is gated on ≥1 declared system OR ≥1 tagged story). This covers `set-release`, `set-system`, `ingest-spec`, `add-item`, `revise` (when it supersedes/splits/merges), `release` (registry reorder/rename), and `migrate-systems`. **`reorder` is exempt** — it changes only `sequence`, which is not a readiness input, so it leaves the views untouched. **In html mode, any op whose write pass re-renders roadmap `.html` pages also (re-)copies the timestamp-parity gate asset** `/roadmap/check-timestamp-parity.cjs` from the skill's `scripts/check-timestamp-parity.cjs`, per the Asset refresh rule in roadmap `SKILL.md` → Timestamp-parity gate — so an existing html roadmap mutated for the first time gains and keeps the gate rather than skipping it (bug-3).
...
- `system list` — print the declared systems (`name` + optional `path`) with a per-system count of stories carrying that band, **and three integrity checks**: (a) **orphans** — any story whose `system` is **non-null but not declared** (e.g. left by a hand-edit) is reported with its id and offending value; (b) **shadowed systems** — any declared system whose `name` collides with a reserved scope word (`roadmap`, `backlog`), a registered release name, or a milestone/phase id/slug is flagged, because the PM bare-`complete <name>` scope can never reach it (the explicit `system:<name>` form still can — see `product-manager/references/scope-resolution.md` → System scope); (c) **invalid paths** — any declared system whose `path` fails the `config.md` → `path` validation rule (e.g. absolute, contains `..`, or carries control characters/newlines from a hand-edit) is flagged as **untrusted / unroutable**, since PM will refuse to surface it (see security note there); (d) **malformed names** — any declared `name` that violates the `config.md` → `name` grammar (e.g. contains whitespace, quotes, angle brackets, or newlines from a hand-edit) is flagged as **untrusted**, since renderers must escape/neutralize it rather than emit it raw (a YAML/HTML-injection vector — fix via `system rename` to a grammar-valid name). Read-only; no diff, no gate.
- `system add <name> [path]` — append a new system to `config.systems`. The safe replacement for hand-editing config. **Name guard:** `<name>` must (a) satisfy the **name grammar** in `config.md` → `name` (`^[a-z0-9]([a-z0-9-]*[a-z0-9])?$`, 1–64 chars) — a value with whitespace, newlines, quotes, angle brackets, or other metacharacters is an **error**, since it is a YAML/HTML-injection vector when emitted into frontmatter, badges, and the readiness matrix; and (b) not collide with (i) an already-declared system, (ii) a **reserved scope word** (`roadmap`, `backlog`), (iii) a **registered release name** in `roadmap.lock.json` → `releases[]`, or (iv) a **milestone/phase id or directory-slug** (e.g. `001`, `001.2`, `001-bootstrap`). A grammar violation or collision is an **error** — the op stops and prints the reason (a collision also explains that the name would be shadowed in the PM bare-`complete <name>` scope, suggesting a different name or the explicit `system:<name>` selector). This keeps every declared system safely renderable and reachable by its bare scope. **Path guard:** a supplied `path` must pass the **path validation rule** in `config.md` → `path` (normalized repo-relative; no absolute/`..`/control-char/newline; portable charset; ≤200 chars). An invalid `path` is an **error** — the op stops and prints the reason (this is a security control: `path` is later surfaced to a command-capable agent, so a malformed value could carry injected instructions). A new (empty) system adds a matrix column, so it re-renders the readiness views (per the apply-contract Write step). Diff marker: `~ changed` (config).
...
1. **Collect the systems declaration (no write yet).** If `config.systems` (see `config.md`) is **empty**, prompt the user to declare systems one at a time — `name` + optional `path` — via structured questions (`AskUserQuestion` in Claude Code, `question` in opencode). Each proposed name is checked against the **`system add` name guard** — both the **name grammar** (`config.md` → `name`: `^[a-z0-9]([a-z0-9-]*[a-z0-9])?$`, no metacharacters/newlines) and the collision check (no reserved word, registered release, or milestone/phase id/slug) — and each supplied `path` against the **`config.md` path validation rule** (normalized repo-relative; no absolute/`..`/control-char/newline); an invalid name or path is rejected and re-prompted. **Hold the collected set in memory** — do **not** write `/roadmap/roadmap.config.json` yet; the config change is part of the single staged diff below and is written only on approval (apply contract: a reject writes nothing). If systems are already declared, skip this step.
2. **Propose per untagged story.** For **every untagged story** (`system: null`, **regardless of status — `done`/`superseded` included**), auto-propose a system by analyzing its `title`, `## Brief`, and parent phase title against the declared system `name`s (the in-memory set from step 1, or the existing `config.systems`) and their `path` hints. Stories already tagged are left untouched (this is what makes re-running idempotent).
3. **One whole-roadmap staged diff.** Present the entire proposal as a **single** staged diff that includes **both** (a) the pending `config.systems` declaration from step 1 — the systems to be written to `roadmap.config.json`, shown as a `~ changed` (config) block — **and** (b) the `⊞ system` story rows, **grouped by proposed system**, so the user can correct any row before applying. Un-inferable stories are shown **untagged** (left `null`) and called out — migration never guesses blindly.
4. **Gate + bulk apply (all writes here, or none).** On **reject → write nothing** (neither config nor story assignments; report the discard). On **approval**, apply in one pass: **first write the collected `config.systems` to `/roadmap/roadmap.config.json`**, then apply every story assignment via `set-system` semantics in bulk — write each story's `system` frontmatter, append one system-change audit row per changed story (`⊞ system`, status unchanged), and update `roadmap.lock.json` `items[].system`. Because this is typically the **first** system tagging on a legacy roadmap, it crosses the standalone dashboard's gate: per the apply contract's Write step, **render the readiness views now** — this **first creates the standalone `/roadmap/release-matrix.<ext>` dashboard** and populates the newly-declared system columns in the **already-present** embedded index matrix (the embedded matrix always exists; before migration it showed a single `(untagged)` column). Then **propose the commit `docs(roadmap): migrate-systems`** (which now includes `roadmap.config.json` and the new `release-matrix.<ext>`). **Never commit.**
