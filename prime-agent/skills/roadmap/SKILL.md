...
**System band.** Every item also carries an optional `system` band (`string | null`) — a **second classification axis orthogonal to both `status` and `release`** (a monorepo story belongs to one deployable system, e.g. `backend`, and to one release train, e.g. `mvp`, at once). It mirrors the `release` band's machinery (nullable per-item field, cascade to not-done descendants, derived phase/milestone badge — `[cross-cutting]` in place of `[mixed]`, editable on frozen items) with deliberate differences: the set of systems is **config-declared** in `roadmap.config.json` → `systems` (not a lazily-grown lock registry) and managed with referential integrity by the `system <list\|add\|rename\|remove>` op (below), systems are an **unordered peer set**, each may carry an optional `path`, and assigning an **undeclared system is an error** (typo guard) — `null` (untag) is always permitted. The band is nullable and lazily written, so legacy roadmaps render and execute unchanged (no badges; no forced migration). The rationale for modeling systems as an orthogonal band (rather than directory structure) and the integrity/namespace obligations it imposes are recorded in [ADR-0001 — Model systems as an orthogonal roadmap band](https://github.com/kterto/my-skills/blob/main/docs/adr/0001-orthogonal-system-band.md). See `references/config.md`, `references/item-schema.md`, and `references/directory-layout.md`.

**Rigor band.** Every story also carries an optional `rigor` band (`string | null`) — a **third orthogonal axis**, and the only one that says what a green on that story **claims** rather than where it belongs: `sketch` (it runs; nothing is claimed about quality), `delivery` (it does what the story's `## Acceptance` says, and the happy path is proven), `hardened` (plus: the gates hold, mutants die, the spec is graded, a human validated what needed validating). `product-manager` forwards it to the orchestrator as `--rigor`, which resolves it into cycle caps and gate block-or-report; a story with no band runs at the project default.

Three deliberate differences from the other two bands: the **vocabulary is closed** (three values and `null` — no `add`, no rename cascade, no orphan or `(unknown)` state, and an unrecognised value is rejected on write); it derives **no** phase/milestone badge, because a phase's mix is already rendered in the release matrix and a fourth badge in every README header is noise; and **lowering it is a recorded event** — the `rigor` op writes a mandatory audit row naming the direction, because a story demoted to `sketch` to get it out the door is a decision someone made and this is the only place it survives the merge. The level contract, including the disclosure set that stays identical at all three levels, is [ADR-0024](../../../../docs/adr/0024-rigor-levels-and-the-invariant-disclosure-set.md). See `references/item-schema.md` and `references/mutation-ops.md`.
...
**Rigor is not a third axis of the matrix — it is one cell per release row.** Each row carries a `rigor` cell holding the release's **mix** (a count per band, most rigorous first: `9 hardened · 3 sketch`, or `—` when no story declares one), because `READY(mvp)` where nine stories ran `delivery` and three ran `sketch` is not one release. A release may declare a `rigor_floor` in `releases[]`; it **never blocks**. A done release with stories below it renders `READY (below floor: n)` plus a non-suppressible departure list naming them — refusing to render READY would put the cheapest path to green through the floor itself, so the matrix states the shortfall instead of hiding it behind a verdict. Still a pure derivation: nothing new is stored beyond the per-story band and the optional floor.
...
`scripts/check-timestamp-parity.cjs` is a **fail-closed** gate that guards against this. It is materialized into `/roadmap/check-timestamp-parity.cjs` in html mode (build Step 5) and **re-copied on every subsequent html-mode write pass** (see the Asset refresh rule below) as a shipped asset. **This skill is doc-only — it writes the gate but never runs it.** The gate is run by CI, by the orchestrator's gating, or by the user:
...
- **Branch scope** (the default) reuses the orchestrator's shared `.orchestrator/gate-scope.cjs` — present whenever the orchestrator/PM stack is bootstrapped alongside the roadmap. Because that helper is refreshed on a **different cadence** than this gate (only when orchestrator setup reruns), the gate is **version-gated** on it: it requires the helper's `SCOPE_API_VERSION` to meet a baked-in minimum and **fails closed** (re-run orchestrator setup, or use `--all`) against an older/absent one, so a hardened gate never runs on a stale helper that silently drops type-changed/dangling paths (arch-1, [ADR-0010 — Version materialized gate runtime dependencies](https://github.com/kterto/my-skills/blob/main/docs/adr/0010-versioned-gate-runtime-dependencies.md)). The `--all` and explicit `--` modes are **self-contained** (no `.orchestrator/` needed, no version gate), so a standalone roadmap can still audit its pages.
- **Asset refresh rule (normative — self-heals upgraded roadmaps).** In html mode the gate asset `/roadmap/check-timestamp-parity.cjs` is (re-)copied from the skill's `scripts/check-timestamp-parity.cjs` in the **same write pass** as **every** lifecycle site that re-renders `.html` pages — not only the initial build. The carrying sites are **Materialize** (build Step 5), the **Sync** and **Re-eval** procedures (`references/sync-and-reeval.md`), and the **mutation-ops apply contract** (`references/mutation-ops.md`). Re-copying identical bytes is a no-op; the point is that a roadmap **built before the gate existed** (or by an older skill version) receives — and keeps current — the asset the first time any html-mode flow touches it, instead of never getting the protection the build-only copy promised (bug-3). Consumers that then *run* the gate (CI, the orchestrator, `product-manager`) must **fail closed** when an html-mode roadmap is missing the asset rather than silently skip — see `product-manager/references/git-flow.md` → Timestamp-parity gate.
  - **Symlink-safe atomic write (sec-1) — normative.** The copy carries **trusted skill bytes into a repository-controlled directory**, so it must never follow a symlink out of the roadmap tree: a symlink pre-planted at `/roadmap/check-timestamp-parity.cjs` (or a symlinked `/roadmap/` parent) would otherwise redirect a naive `cp`/overwrite to clobber a file **outside** the roadmap with the writer's privileges. The refresh is therefore, in order: (1) **canonical-containment check** — resolve the real path of the roadmap parent directory (`realpath`) and require it to be the repository's own `roadmap/` directory; refuse if the parent is a symlink escaping the repo. (2) **Destination type check** — `lstat` the destination; if it exists and is **not a regular file** (a symlink, directory, device, …), **refuse and report** — never write *through* it. (3) Write the trusted bytes to a **same-directory temporary regular file** (e.g. `check-timestamp-parity.cjs.tmp-<pid>`). (4) **Re-check** the destination is still absent-or-a-regular-file (TOCTOU guard). (5) Atomically **`rename`** the temp file onto the destination. Never write or `cp` directly onto the destination path, which follows a pre-planted symlink. A failure at any step aborts the write pass with a reported error rather than proceeding.
