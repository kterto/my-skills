...
function runG6(files, stackCfg, io, deps = {}) {
  ...
  const viaMutationTest = () => {
    ...
    // A refused run and a run we killed on the clock are both unmeasured: the
    // report on disk after a kill covers an arbitrary prefix of the scope, so it
    // is discarded rather than scored.
    if (run.unmeasured || run.timedOut) {
      return g6Unmeasured({ ...run, reason: run.reason || 'run-timeout' }, opts);
    }
