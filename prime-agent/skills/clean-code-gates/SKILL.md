...
- **G6 (mutation, dart-flutter)** runs the `mutation_test` pub package by default. It writes a
  ...
  `dart pub global run mutation_test -f junit`, and derives the score from the junit report:
  `<testsuite tests= failures=>` sums to total/undetected, and each failing `<testcase>` names
  its file and line. junit is the only format carrying both halves — the plain `xml` report
  lists undetected mutations with no total, so no score can be derived from it. When
  ...
  the single source of every number. Both tools sandbox their mutations and leave no report
  directory, worktree, or `pub get` litter behind.
  Note `mutation_test` edits target files **in place** and restores them on a clean exit,
  where `dart_mutant` sandboxes its mutations. The adapter snapshots every target before the
  run and puts back anything that differs afterwards, so a crash or a killed tool cannot leave
  a live mutation in the tree; a `SIGKILL` to the gate process itself is the documented residual.