...
- `git status --porcelain=v1 -- ':(exclude).opencode' ':(exclude).claude' ':(exclude).prime'` — staged/unstaged state. The excludes drop host-runtime scaffolding (opencode's `.opencode/`, Claude Code's `.claude/`); without them the tree reads permanently dirty under those hosts and the PR-only fast-path never triggers.
...
- `git diff --stat HEAD -- ':(exclude).opencode' ':(exclude).claude' ':(exclude).prime'` — overview of changed files
- `git diff --cached -- ':(exclude).opencode' ':(exclude).claude' ':(exclude).prime'` and `git diff -- ':(exclude).opencode' ':(exclude).claude' ':(exclude).prime'` — actual content
