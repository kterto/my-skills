<!--
FIXTURE PROVENANCE: GIT-RECOVERED. Commit 678ed56, file
prime-agent/overlays/protocol.rlm-dispatch.md, reproduced verbatim below this
header.

Instance 4 of the emitted-fence defect class: generator exhaustion. The wave
fence reads its job list TWICE — once to admit the children and once to build
the name map — while the prose above it permitted a generator ("where jobs is
one (name, prompt) pair per child, built before the call"). A generator is
exhausted by the gather, so the name map is built empty and the retry path
below raises instead of reaching the documented fallback.

WEAK EVIDENCE, BY MEASUREMENT. The fence bytes are BYTE-IDENTICAL before and
after this defect's fix; the entire emitted change across 678ed56..a730a73 is
prose. No fence-body rule can reach this instance. It is caught only by PF04, a
prose-phrasing pattern rated weak: PF04 is validated against exactly two known
phrasings and is gameable by writing "list" without meaning it.

EXPECTED RULE IDS: PF04, and no other.
-->
