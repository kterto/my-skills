...
Since each session starts clean, **invoke or load the `spec-driven-eval` skill in the eval sessions (1 and 4)** — they follow it. Sessions 2 and 3 follow the *framework-under-test's* own instructions.
...
Open a new chat (with the `spec-driven-eval` skill loaded) and paste:

```
Following this spec-driven-eval skill, do ONLY the baseline-freeze steps
...
## Session 4 — Evaluate (the skill)

Open a new chat on the implemented branch (with the `spec-driven-eval` skill loaded):

```
Following this spec-driven-eval skill, grade the implementation on the current
